                  ...
                  child: ConstrainedBox(
                    ...
                    child: DataTable(
                      ...
                      rows: parkings.map((parking) {
                        debugPrint(
                          'Parking ID: ${parking.id}, End Time: ${parking.endTime}, Is After Now: ${parking.endTime != null ? parking.endTime!.isAfter(DateTime.now()) : 'N/A'}',
                        );
                        ...
                        return DataRow(
                          ...
                          cells: [
                            DataCell(
                                Text(parking.vehicle?.licensePlate ?? 'N/A')),
                            DataCell(
                                Text(parking.parkingSpace?.address ?? 'N/A')),
                            DataCell(
          ...
          if (currentState is ParkingLoaded) {
            ...
            final availableVehicles = currentState.vehicles.where((vehicle) {
              return !ongoingSessions
                  .any((session) => session.vehicle?.id == vehicle.id);
            }).toList();

            final availableParkingSpaces =
                currentState.parkingSpaces.where((space) {
              return !ongoingSessions
                  .any((session) => session.parkingSpace?.id == space.id);
            }).toList();
          ...
          if (currentState is ParkingLoaded &&
            ...
            showDialog(
              ...
              builder: (context) => AlertDialog(
                ...
                content: Text(
                    'Do you want to delete the parking session for vehicle "${selectedParking.vehicle?.licensePlate}"?'),
                actions: [
                  TextButton(
